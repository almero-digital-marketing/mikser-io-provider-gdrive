...
export function rememberFile({ fileId, folderId, entityId, name, mimeType, modifiedTime }) {
    useDatabase().handle.prepare(`
        INSERT INTO mikser_provider_gdrive_files
            (file_id, folder_id, entity_id, name, mime_type, modified_time, last_seen_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    `).run(fileId, folderId, entityId, name, mimeType ?? null, modifiedTime ?? null, Date.now())
}
